$(document).ready(function () {
  ...
  $("#savenewaddress").on("click", function (e) {
    ...
    if (streetname.length < 1){
      $("#streetname").after("<span class='error'>*Field can't be empty</span>");isValid =false;
    }
    if(housenumber.length < 1){
      $("#housenumber").after("<span class='error'>*Field can't be empty</span>");isValid =false;
    }

    if(phonenumber.length < 1){
      $("#phonenumber").parent().after("<div style='margin-bottom:10px'><span class='error'>*Field can't be empty</span></div>");isValid =false;
    }else if(!reg.test(phonenumber)) {
      $("#phonenumber").parent().after("<div style='margin-bottom:10px'><span class='error'>*numbe must be 10 character long</span></div>");isValid =false;
    }
